...
matches ws = map length . filter (endswith ws) . inits  -- (ec.40)
\end{minted}

% TODO: explicar porque di esta definición
...
La función \hsCode{endswith ws xs} tiene un valor \hsCode{Bool}, regresa verdadero si \texttt{ws} es un algún sufijo de \texttt{xs}.
% TODO: poner que se omitirá por ahora como va endwith
...
Siempre hay que tener en cuenta que cualquier problema que involucre la función \hsCode{inits}, la ley más importante es conocida como \texttt{Scan Lemma}:
% map (foldl op e) · inits = scanl op e
% TODO: esto xplicarlo mucho mejor en otro capçitulo, poner como de qué cada lado de la ecuación y en el capítulo poner la prueba

Aunque hay un \hsCode{map} en la definición de \hsCode{matches}, también hay un \hsCode{filter}, así que el primer paso en tranformar \hsCode{matches} is reescribir la especificación usando otra ley:
% TODO: demostrar esta ecuación
\begin{minted}{haskell}
map f . filter p = map fst . filter snd . map (fork (f,p))  -- (ec.42)
\end{minted}

donde \hsCode{fork (f , p) x = (f x, p x)}. La ley usada simplemente es para llevar a \hsCode{map} después de \hsCode{inits} para prepararlo para aplicar el \textit{scan lemma}. Usando (ec.40) nos lleva a
...
% TODO: poner esto de abajo en la introducción de que muhcas veces no sabemos a donde vamos, pero no es misterioso.
% Again, the focus is on the form of the terms in the specification, not on what they do nor on the shape of the final algorithm. It may be a little scary not to know where we are heading, but it is not mysterious.

La siguiente pregunta a responder es: ¿Puede \hsCode{fork (length, endswith ws)} puede ser casteado como una instancia de \hsCode{fold}? Definitivamente \hsCode{length = fold count 0} donde \hsCode{count n x = n+1} % TODO: demoostrar esa pendejada
Supongamos que se pueden encontrar \hsCode{e} y \hsCode{op}, que cada uno de ellos dependa de \texttt{ws} tal que
...
\begin{minted}{haskell}
endswith ws = foldl op e  -- (ec.43)
\end{minted}

% TODO: demostrar esto
Entonces se está en posición de aplicar otra ley: la \textit{tupling law} para \hsCode{foldl}. La ley establece que,
\begin{minted}{haskell}
fork (foldl op1 e1, foldl op2 e2) = foldl op (e1, e2)  -- (ec.44)
\end{minted}

donde \hsCode{op (a,b) x = (op1 a x, op2 b x)}. Usando \textit{tupling law} resulta en
...
Si \hsCode{op} toma tiempo constante o al menos amortizada, y también \hsCode{step}, y el resulrado es tiempo lineal.

El problema ahora es que no hay \hsCode{e} y \hsCode{op} que satisfagan (ec.43). La función de \hsCode{endswith ws} regresa un \hsCode{Bool} y eso es información insuficiente para expresar
a \hsCode{endswith ws} como una composición

\begin{minted}{haskell}
endswith ws = p . foldl op e  -- (ec.44)
\end{minted}

La forma de (ec.44) está puesta de esta manera solamente para aplicar el \textit{scan lemma}. En ves de (ec.42) usando una ligera generalización:

\begin{minted}{haskell}
matches ws = map fst . filter (p . snd) . scanl step (0,e)
\end{minted}

Dadas \hsCode{p} y \hsCode{op} take amortized constant time, matches will still take linear time.

Lo que queda aún no se ha definido formalmente es \hsCode{endswith}, una definición razonables podría ser:
