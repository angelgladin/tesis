...
% TODO: ver si pongo estot
La razón de elegir \hsCode{rep} en la (eq.10) es que \hsCode{op'} toma una forma más simple.

Por ánalis de casos en \hsCode{op'},
\begin{minted}{haskell}
op' (Node (us, vs) l r) x
...
r
\end{minted}
La primera cláusula quedando como \hsCode{[x] ⊑ vs  = r}. % cambiarla
...
root
\end{minted}
La segunda cláusula quedando como\hsCode{ null us = rep([], ws) = root -- (eq.2964)}. % cambiarlo
...
op' l x
\end{minted}
La tercera cláusula %TODO

% TODO: ponerlos hasta abajo
\begin{minted}{haskell}
op' (Node (us, vs) l r) x
  | [x] ⊑ vs  = r
  | null us   = root
...
Las otras cláusulas son similares, Si ponemos \hsCode{op' Null x = root}, entonces \hsCode{op'} toma una forma aún más sencilla.
\begin{minted}{haskell}
op' Null x                 = root
...
% FIXME de aquí para abajo checar como va todo.
% TODO poner bien la explicación de pag 131.

La idea es dar una versión generalizanda, digamos \hsCode{grep} de \hsCode{rep}.

\begin{minted}{haskell}
rep (us, vs) = grep (left us vs) (us, vs)
\end{minted}

Y luego obtener una definición de \hsCode{grep}. De (eq.10) se tiene
...
Para simplificar \hsCode{left (us ++ [v]) vs} se necesitas hacer análisis de casos en \hsCode{us}.
...
% Poner lo que hace falta por escribir
